...
void bta_hf_client_send_at_xapl(void)
{
    char *buf;

    APPL_TRACE_DEBUG("%s", __FUNCTION__);
    //Set docker and battery level service
    buf = "AT+XAPL=0505-1990-0100,6\r";

    bta_hf_client_send_at(BTA_HF_CLIENT_AT_XAPL, buf, strlen(buf));
}

void bta_hf_client_send_at_iphoneaccev(UINT8 battery_level, BOOLEAN docked)
{
    char *buf = osi_malloc(BTA_HF_CLIENT_AT_MAX_LEN);
    ...
    at_len = snprintf(buf, BTA_HF_CLIENT_AT_MAX_LEN, "AT+IPHONEACCEV=2,1,%u,2,%u\r", battery_level, docked);

    bta_hf_client_send_at(BTA_HF_CLIENT_AT_IPHONEACCEV, buf, at_len);
    osi_free(buf);
}
