...
esp_err_t esp_hf_client_send_xapl(void)
{
    if (esp_bluedroid_get_status() != ESP_BLUEDROID_STATUS_ENABLED) {
        return ESP_ERR_INVALID_STATE;
    }
...
esp_err_t esp_hf_client_send_iphoneaccev(uint8_t battery_level, bool docked)
{
    if (esp_bluedroid_get_status() != ESP_BLUEDROID_STATUS_ENABLED) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    memset(&arg, 0, sizeof(btc_hf_client_args_t));
    arg.iphone_headset_state_param.battery_level = battery_level;
    arg.iphone_headset_state_param.docked = docked;
