...
DRAM_ATTR StaticTask_t xBtuTaskStruct;
...
void BTU_StartUp(void)
{
    ...
    BtuStack = osi_malloc(BTU_TASK_STACK_SIZE);
    if (!BtuStack) {
        goto error_exit;
    }
    xBtuTaskHandle = xTaskCreateStaticPinnedToCore(btu_task_thread_handler, BTU_TASK_NAME, BTU_TASK_STACK_SIZE, NULL, BTU_TASK_PRIO, BtuStack, &xBtuTaskStruct, BTU_TASK_PINNED_TO_CORE);
