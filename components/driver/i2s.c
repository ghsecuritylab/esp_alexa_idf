...
static int tx_buffer_flag = false;
...
void i2s_set_tx_buffer_flag()
{
    tx_buffer_flag = true;
}

void i2s_reset_tx_buffer_flag()
{
    tx_buffer_flag = false;
}
...
static void IRAM_ATTR i2s_intr_handler_default(void *arg)
{
    ...
    if (i2s_reg->int_st.out_eof && p_i2s->tx) {
        ...
        if (tx_buffer_flag == true) {
            memset(finish_desc->buf, 0xFF, finish_desc->size);
        }
        if (xQueueIsQueueFullFromISR(p_i2s->tx->queue)) {
            ...
            if (p_i2s->tx_desc_auto_clear == true) {
                if (tx_buffer_flag == true) {
                    memset((void *) dummy, 0xFF, p_i2s->tx->buf_size);
                } else {
                    memset((void *) dummy, 0x00, p_i2s->tx->buf_size);
                }
            }
        }
